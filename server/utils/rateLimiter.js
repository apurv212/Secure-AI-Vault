 ...
 * Allows 10 extractions per hour per user
 * 
...
const extractionLimiter = rateLimit({
  ...
  max: 10, // Only 10 extractions per hour
  message: {
    error: 'Too many extraction requests',
    message: 'You can only perform 10 card extractions per hour',
    retryAfter: 3600
  },
  standardHeaders: true,
  legacyHeaders: false,
  handler: (req, res) => {
    ...
    res.status(429).json({
      error: 'Too many extraction requests',
      message: 'You have reached the maximum number of card extractions (10 per hour). This limit helps us manage costs and prevent abuse.',
...
const uploadLimiter = rateLimit({
  ...
  message: {
    ...
    message: 'You can only upload 20 files per hour',
    retryAfter: 3600
  },
  standardHeaders: true,
  legacyHeaders: false,
  handler: (req, res) => {
    ...
    res.status(429).json({
      error: 'Too many upload requests',
      message: 'You have reached the maximum number of file uploads (20 per hour).',
      retryAfter: 3600
    });
  }
});
 ...
 * Allows 50 operations per 15 minutes
 */
const cardOperationsLimiter = rateLimit({
  ...
  message: {
    ...
    message: 'Please slow down. You can perform up to 50 card operations every 15 minutes',
    retryAfter: 900
  },
  standardHeaders: true,
  legacyHeaders: false,
